...
const wcagData = JSON.parse(
  readFileSync(join(__dirname, "..", "data", "wcag.json"), "utf-8"),
);
...
if (sc247) {
  ...
  if (sc247.understanding?.brief) {
    ...
    console.log(
      `  Why it's important: ${sc247.understanding.brief["why it's important"]}`,
    );
  }
  ...
  if (sc247.understanding?.benefits) {
    ...
    console.log(
      `  First: ${sc247.understanding.benefits[0].substring(0, 100)}...`,
    );
  }
...
console.log(
  `Success Criteria with Understanding content: ${totalWithUnderstanding} / 87`,
);
