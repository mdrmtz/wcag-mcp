...
async function fetchWcagJson() {
  ...
  const scCount = wcag.principles.reduce(
    (sum, p) =>
      sum +
      p.guidelines.reduce((gSum, g) => gSum + g.successcriteria.length, 0),
    0,
  );
  ...
  console.log(
    `  → ${wcag.principles.reduce((sum, p) => sum + p.guidelines.length, 0)} guidelines`,
  );
