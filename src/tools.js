...
const getSuccessCriteriaDetail = {
  ...
  inputSchema: {
    ...
    properties: {
      ref_id: {
        type: "string",
        description:
          'Success criterion reference number (e.g., "1.1.1", "2.4.7")',
      },
    },
    required: ["ref_id"],
  },
  handler: async (args) => {
    ...
    if (!result) {
      return textResponse(
        `No success criterion found with number "${args.ref_id}". Use format like "1.1.1" or "2.4.7".`,
      );
    }
...
const getCriteriaByLevel = {
  ...
  handler: async (args) => {
    ...
    const levels = args.include_lower
      ? levelHierarchy[args.level]
      : [args.level];
...
const countCriteria = {
  ...
  handler: async (args) => {
    ...
    return textResponse(
      `# WCAG 2.2 Success Criteria by ${args.group_by.charAt(0).toUpperCase() + args.group_by.slice(1)}\n\nTotal: ${allCriteria.length} success criteria\n\n${output}`,
    );
  },
};
...
const listTechniques = {
  ...
  handler: async (args) => {
    ...
    if (args.technology) {
      const tech =
        args.technology === "failures" ? "failures" : args.technology;
...
const getTechniquesForCriterionTool = {
  ...
  handler: async (args) => {
    ...
    if (!result) {
      return textResponse(
        `No success criterion found with number "${args.ref_id}".`,
      );
    }
...
const searchTechniques = {
  ...
  handler: async (args) => {
    const query = args.query.toLowerCase();
    const techniques = getAllTechniques().filter(
      (t) =>
    ...
    const output = techniques
      .map((t) => `**${t.id}** (${t.technology}): ${t.title}`)
      .join("\n");
...
const getFailuresForCriterion = {
  ...
  handler: async (args) => {
    ...
    if (failures.length === 0) {
      return textResponse(
        `No documented failure techniques for ${sc.num} ${sc.handle}.`,
      );
    }
  ...
  handler: async (args) => {
    ...
    if (!term) {
      ...
      if (similar.length > 0) {
        ...
        return textResponse(
          `Term "${args.term}" not found. Did you mean:\n\n${suggestions}`,
        );
      }
      return textResponse(
        `Term "${args.term}" not found in the WCAG glossary.`,
      );
    }
